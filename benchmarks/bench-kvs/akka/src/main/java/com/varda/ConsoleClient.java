...
public class ConsoleClient {
    ...
    private static class State {
        MaingRPCClient grpcClient = null;
    }
...
//                .variable(LineReader.SECONDARY_PROMPT_PATTERN, "%M%P > ")
        ...
        terminal.writer().println("\t varda Console");
        terminal.writer().println("-----------------------------");
        terminal.flush();
...
//                terminal.flush();
                ...
                if ("connect".equals(pl.word())) {
                    ...
                    if (pl.words().size() == 2) {
                        ...
                        terminal.writer().println("get: "+currentState.grpcClient.get(key));
                    } else {
                    ...
                    if (pl.words().size() == 3) {
                        ...
                        terminal.writer().println("put: "+currentState.grpcClient.put(key, value));
                    } else {
    ...
    public static MaingRPCClient connect(String ip, String port) {
        assert (null != ip && !ip.isEmpty());
        assert (null != port && !port.isEmpty());

        return new MaingRPCClient(ip, Integer.parseInt(port));
    }

    private static void checkState(State state, boolean client) {
        if (client && null == state.grpcClient) {
            throw new StateException("Not connected to varda !");
        }
    }
