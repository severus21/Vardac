...
class LoCCounter(Counter):
    ...
    def __init__(self, name, target, exports, excludes=[]) -> None:
        super().__init__(name, target)
    ...
    def count(self):
        with NamedTemporaryFile("w+", delete=False) as fp:
            with open(CLOC_EXCLUDES, "r") as fp2:
                for line in fp2:
                    fp.write(line)
            for exclude_file in self.excludes:
                fp.write("\n"+os.path.abspath(os.path.join(self.target, exclude_file))+"\n")

            fp.flush()

            res = subprocess.run(
                f"cloc . --json --force-lang-def={CLOC_DEFINITIONS} --exclude-list-file={fp.name} --exclude-content={CLOC_EXCLUDE_TOKEN}",
                capture_output=True,
                encoding='utf-8',
                shell=True,
                cwd=self.target)
