...
module Make () = struct
    ...
    *)
    let rec aux_reset_times_ac env place st0 (guard_headers, guard_opt) = 
        ...
        logger#debug "updated_registered %s" (List.fold_left (fun acc x -> acc ^ " " ^(Atom.to_string x)) "" updated_registered_timers); 
        print_env new_env1;
        ...
        | Some guard -> begin
            ...
            *)
            ...
            logger#debug "timer_to_resets %s" (List.fold_left (fun acc x -> acc ^ " " ^(Atom.to_string x)) "" timers_to_reset); 
