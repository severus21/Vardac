...
module InterceptedActivationSet = Set.Make(struct 
    type t = atom * atom (* (schema, activation variable) *)
    let compare (a, b) (c, d) = 
...
let intercepted_schemas_of_stmt place cname stmt = 
    ...
    let intercepted_schemas = collect_stmt_stmt None spawn_selector spawn_collector stmt in
...
let intercepted_bridges_of_ctx place cname stmts : InterceptedActivationSet.t = 
    let fvars = List.amp (function stmt -> snd (free_vars_stmt Atom.Set.empty stmt)) stmts
...
let generate_onboard_typedef place cname intercepted_schemas = 
    let onboard_st_branch_of schema = 
        ...
        schema, auto_fplace (STSend(
            mtype_of_ct (TTuple [
                mtype_of_ct (TActivationRef (mtype_of_cmt (CompTUid schema))),
                mtype_of_ft (TPlace)
            ]),
...
let insert_interceptor_dcl key p_onboard_def interceptor_dcl (program:program) : program =
...
let mt_internals_of place intercepted_schemas = 
    ...
    List.fold_left (fun mt schema -> mtype_of_ct (TUnion mt (mtype_of_cmt (ComTUid schema))) (mtype_of_cmt (ComTUid (List.hd intercepted_schemas))) (List.tl intercepted_schemas) in
...
let generate_bridges place interceptor_name intercepted_schemas p_onboard intercepted_bridges =
    ...
    let b_onboard_let = LetExpr (
        mtype_of_ft (TBridge {
            ...
            out_type = mtype_of_cmt (CompTUid interceptor_name);
            protocol = p_onboard;
        }),
        b_onboard,
...
(*** Step 1.C - Insert new onboard protocol in lca of intercepted_schemas ***)
let insert_interceptor_dcl key p_onboard_def interceptor_dcl (program:program) : program =
    let parents = Hashtbl.find interceptor_parents key in
...
let generate_interceptor_factory place interceptor base_interceptor_constructor_params (generated_bridges, (b_onboard, b_onboard_let))  = 
    ...
    let spawn = 
        Spawn {
            c = auto_fplace(CompTUid interceptor, auto_fplace EmptyMainType);
            args = 
...
let ctxelim_rewrite_stmt program place = function 
    | WithContextStmt (anonymous_mod, cname, user_defined_policy, stmts) -> 
        ...
        *)
        ...
        (* 
            a -> (p_of_a_opt, a', i_a) where a' is the new name of a inside the ctx - in order not to leak it and i_a is the identity of the interceptor in charge of a'.
            Whereas a is the proxy of a' for the outside if exposed.
        *)
        let intercepted_activations_info = Hashtbl.create 16 in
        ...
        let spawn_selector = function
            ...
            | _ -> false
        in
        (* rewrite intercepted_spawn and hydrate intercepted_activations_info *)
        let rewriter parent_opt place = function
            | Spawn spawn -> begin 
                ...
                | VarCExpr c -> 
                    ...
                    TODO need to add i_a and p_of_a just befor stmt containing this
                    maybe to nested rewriter one for stmt and one for expr


                    
                    Hashtbl.add intercepted_activations_info a (
                        Option.map (function _ -> p_of_a) spawn.at, a', i_a 
                    );
                    ...
                    LetExpr(
                        auto_fplace (TActivationRef (mtype_of_cmt (CompTUid interceptor_name))),
                        i_a,
                        auto_fplace (CallExpr(
                            intercepted_activations_info, 
            ...
            | _ -> []
        in 
        (* Hydrate intercepted_activations_info *)
        List.iter (function stmt -> ignore (collect_stmt_stmt None spawn_selector spawn_collector stmt)) stmts;
...
(****************** CTX Elim ************************)
    ...
    if intercepted_bridges/schemas = [] then
        Error.error place "Interception context intercepts no component schema !!";
...
let scope_rewriter terms = 
    ...
    let terms = IR_utils.insert_in_terms 
        (List.map (function c -> auto_fplace (Component c)) (List.of_seq (Hashtbl.to_seq_values interceptor_makes)))
        terms
    in
