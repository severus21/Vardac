...
| CustomMethod ({ghost;} as m) -> begin  
    ...
    *)
    ...
    | (CallExpr ({value=VarExpr x}, [s; bridge]) as e) when Atom.is_builtin x && Atom.hint x = "receive" ->
        ...
        [
            auto_fplace (LetExpr (
                auto_fplace (CType (auto_fplace(TVar (Atom.fresh_builtin "TODO")))),
                tmp, 
                recv)
            );
        ], e
    ...
    | {place; value=LetExpr ({value=CType{value=TTuple [msg_t;{value = SType continuation_st}]}}, _, {value=CallExpr ({value=VarExpr x}, [s; bridge]) as e})}::stmts  when Atom.is_builtin x && Atom.hint x = "receive" -> 
        ...
        let intermediate_args = m.args @ (List.map 
            ...
            intermediate_args
        ) in (* FIXME TODO REMOVE type hardcoding + m.value.args sould have been captured by freee_vars if needed -> can be removed when type annotations *)
